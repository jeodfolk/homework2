...
function begin_play(){
	...
	var p1empty = isEmpty(player1.value);
	var p2empty = isEmpty(player2.value);
	if (started == false)
		{
		if(p1empty && p2empty)
			{
		...
		{
		alert ("Click Reset Play button to start a new game")
		}
}
...
*/
function gameover(player,next)
	{
	if((board_state[0] == turn && board_state[1] == turn && board_state[2] == turn) ||
	   (board_state[3] == turn && board_state[4] == turn && board_state[5] == turn) ||
	   (board_state[6] == turn && board_state[7] == turn && board_state[8] == turn) ||
	   (board_state[0] == turn && board_state[3] == turn && board_state[6] == turn) ||
	   (board_state[1] == turn && board_state[4] == turn && board_state[7] == turn) ||
	   (board_state[2] == turn && board_state[5] == turn && board_state[8] == turn) ||
	   (board_state[0] == turn && board_state[4] == turn && board_state[8] == turn) ||
	   (board_state[2] == turn && board_state[4] == turn && board_state[6] == turn))
	   {
		...
		{
		document.getElementById("turn_info").innerHTML = "Turn for: <b>"+next+"</b>";
		return;
		}
...
function play() {
	...
	var currplay = document.getElementById("move_text_id").value;
	if(!currplay.match(validmove))
		{
		alert("Enter a valid move. EX [A, B, or C][1, 2, or 3]")
		currplay ="";
		return;
		}
	...
	for(i = 0; i < table_ids.length; i++)
		{
		if(currplay == table_ids[i])
			{
			if(board_state[i] == 1 || board_state[i] == 0)
				{
				alert ("Board position already played");
				return;
				}
			else
				{
				board_state[i] = turn;
				document.getElementById(currplay).innerHTML = player
				currplay ="";
				break;
				}
			}
